...
inspect affected helpers and callers. Synchronous Client Components can use
React `use`; do not make every component async. `UnsafeUnwrapped` casts and
codemod error comments require review rather than removal.
...
Follow [NextRequest Geolocation](#nextrequest-geolocation) for your deployment
provider. Do not assume the app is deployed on Vercel.
