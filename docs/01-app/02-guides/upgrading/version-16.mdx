...
Follow [Async Request APIs](#async-request-apis-breaking-change) and inspect
affected helpers and callers. Resolve codemod markers rather than deleting them
or adding casts.

### A webpack configuration error appears under Turbopack

Inspect plugins as well as `next.config`, then choose a documented Turbopack
option or explicitly select webpack when the app still requires it. Do not
discard required configuration merely to pass the build.

### The image optimizer returns `400 Bad Request`

Inspect the failing URL against the relevant [`next/image` changes](#nextimage-changes)
before changing configuration. Avoid broadly disabling image restrictions.
